#!/usr/bin/env npx tsx
/**
 * Update diagram fragments with bilingual mermaid sources
 * English in content_en, Japanese in content_ja
 */
...
// Bilingual diagram definitions
const diagrams: Record<string, { en: string; ja: string }> = {
...
async function main() {
  const sqlStatements: string[] = [
    '-- Update Password Vault Diagram Fragments with Bilingual Content',
    '-- English in content_en, Japanese in content_ja',
    '-- Generated: ' + new Date().toISOString(),
    '',
  ];

  for (const [id, sources] of Object.entries(diagrams)) {
    console.log(`Processing ${id}...`);

    const svgPath = `diagrams/${id}.svg`;
    const contentEn = createMermaidHtml(sources.en, svgPath);
    const contentJa = createMermaidHtml(sources.ja, svgPath);
...
WHERE id = '${escapeSQL(id)}';`;
  ...
  const outputPath = 'scripts/update-diagram-bilingual.sql';
