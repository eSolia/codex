...
export const load: PageServerLoad = async ({ params, platform, locals }) => {
  ...
  try {
    ...
    const [sitesResult, typesResult, fragmentsResult] = await Promise.all([
      ...
      db
        .prepare(
          "SELECT id, name, slug, category FROM fragments ORDER BY category, name"
        )
        .all(),
    ]);
...
export const actions: Actions = {
  ...
  publish: async ({ params, platform, locals }) => {
    if (!platform?.env?.DB || !platform?.env?.CODEX_BUCKET) {
      return fail(500, { message: "Storage not available" });
    }

    const db = platform.env.DB;
    const bucket = platform.env.CODEX_BUCKET;
    const actor = locals.user?.email || "anonymous";
    ...
    try {
      ...
      if (
        ...
        Date.now() / 1000 < content.embargo_until
      ) {
