...
{#if compact}
  ...
  <div class="workflow-progress-compact flex items-center gap-1">
    {#each sortedStages as stage, index (stage.id)}
      ...
      >
        <div
          class="w-6 h-6 rounded-full flex items-center justify-center {colors.circle}"
        >
          {#if status === "completed"}
        ...
        {#each sortedStages as stage, index (stage.id)}
          ...
          <li class="relative {!isLast ? 'pb-8' : ''}">
            {#if !isLast}
              <!-- Connector line -->
              <div
                class="absolute left-4 top-8 -ml-px h-full w-0.5 {status === 'completed' ? 'bg-green-500' : 'bg-gray-200'}"
                aria-hidden="true"
              ></div>
            {/if}
            ...
            <div class="relative flex items-start group">
              ...
              <span class="flex h-8 items-center">
                ...
                >
                  ...
                  {:else}
                    <svg class="w-4 h-4 {colors.icon}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d={getStageIcon(stage.stageType)} />
                    </svg>
                  {/if}
                </span>
              </span>
