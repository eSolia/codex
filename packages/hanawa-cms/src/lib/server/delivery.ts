...
export function createDeliveryService(
  ...
  return {
    ...
    async invalidateCache(collection: string, slug?: string): Promise<void> {
      ...
      const prefix = slug ? `delivery:${collection}:${slug}:` : `delivery:${collection}:`;
    ...
    async listApiKeys(): Promise<ApiKey[]> {
      const { results } = await db.prepare('SELECT * FROM api_keys ORDER BY name').all();
    ...
    async revokeApiKey(id: string, context: AuditContext): Promise<void> {
      await db.prepare('UPDATE api_keys SET enabled = 0 WHERE id = ?').bind(id).run();
    ...
    buildCacheKey(collection: string, slugOrId: string, options: DeliveryOptions): string {
    ...
    rowToApiKey(row: Record<string, unknown>): ApiKey {
      return {
        ...
        allowed_origins: row.allowed_origins ? JSON.parse(row.allowed_origins as string) : null,
