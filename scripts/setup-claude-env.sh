...
if [ -f "$MCP_TEMPLATE" ]; then
  if [ -f "$MCP_TARGET" ]; then
    ...
    echo "4. Copied .mcp.json template (edit to add your Worker URL and secret)"
  fi
else
...
echo "Next steps:"
echo ""
echo "  1. Edit .mcp.json to set your Worker URL and shared secret"
echo ""
echo "  2. Or add the MCP server to your user-scoped config:"
echo "     claude mcp add esolia-standards --transport sse \\"
echo "       --url https://esolia-standards-mcp.YOUR_SUBDOMAIN.workers.dev/sse \\"
echo "       --header 'Authorization: Bearer YOUR_SECRET'"
echo ""
